...
const PRODUCT_LAST_COLUMN = "K";
const VENDOR_LAST_COLUMN = "L";
const PRODUCT_SELECTOR_RANGES = [
  `${PRODUCT_SHEET}!A:A`,
  `${PRODUCT_SHEET}!B:B`,
  `${PRODUCT_SHEET}!C:C`,
  `${PRODUCT_SHEET}!D:D`,
...
async function listAll(
  ...
  let page = 1;
  let totalPages = 1;
  const effectivePageSize = Math.min(100, Math.max(10, pageSize));
  const seenPages = new Set();
  while (page <= totalPages) {
    if (page > 10000) throw new Error(`${call} excedeu o limite seguro de paginas`);
    ...
    if (!Array.isArray(response[listKey])) {
      throw new Error(`${call} retornou lista invalida`);
    }
    rows.push(...response[listKey]);
    const responsePage = Number(response.pagina || response.nPagina || page);
    if (seenPages.has(responsePage)) throw new Error(`${call} repetiu uma pagina`);
    seenPages.add(responsePage);
    totalPages = Number(response.total_de_paginas || response.nTotPaginas || 1);
    if (!Number.isInteger(totalPages) || totalPages < page) throw new Error(`${call} retornou paginacao invalida`);
    page++;
  }
  return rows;
}
...
async function listAllNumbered(
  ...
  while (page <= totalPages) {
    ...
    rows.push(...response[listKey]);
    const responsePage = Number(response.nPagina || page);
    if (seenPages.has(responsePage)) throw new Error(`${call} repetiu uma pagina`);
    seenPages.add(responsePage);
...
function replacementRequests({
  ...
  const sorted = [...deleteIndexes].sort((a, b) => a - b);
  const blocks = [];
  ...
  for (const block of blocks.reverse()) {
    requests.push({
      deleteDimension: {
        range: { sheetId, dimension: "ROWS", startIndex: block.start, endIndex: block.end },
      },
    });
  }
  if (newRows.length) {
...
async function readSelectorRows(sheets) {
  const [
    productDates,
    productCompanies,
    productInvoices,
    productDescriptions,
    productAmounts,
  ] = await sheets.getValuesBatch(PRODUCT_SELECTOR_RANGES, {
  ...
  const [
    ...
    vendorAmounts,
    vendorCompanies,
  ] = await sheets.getValuesBatch(VENDOR_SELECTOR_RANGES, {
    valueRenderOption: "UNFORMATTED_VALUE",
    dateTimeRenderOption: "SERIAL_NUMBER",
  });

  return {
    productRows: selectorRows(
      productDates,
      productCompanies,
      productInvoices,
      productDescriptions,
...
function windowMetrics(rows, indexes, amountColumnIndex) {
  return {
    count: indexes.length,
    amount: Math.round(indexes.reduce((sum, index) => sum + toNumber(rows[index]?.[amountColumnIndex]), 0) * 100) / 100,
  };
}

function rowsFingerprint(rows, indexes, { dateColumnIndex, amountColumnIndex, keyColumnIndexes }) {
  const signatures = indexes.map((index) => {
    const row = rows[index] || [];
    return keyColumnIndexes.map((columnIndex) => {
      if (columnIndex === dateColumnIndex) return parseDateValue(row[columnIndex]) || "";
      if (columnIndex === amountColumnIndex) return Math.round(toNumber(row[columnIndex]) * 100);
      return normalizeText(row[columnIndex]);
    });
  });
  signatures.sort((left, right) => JSON.stringify(left).localeCompare(JSON.stringify(right), "pt-BR"));
  return crypto.createHash("sha256").update(JSON.stringify(signatures)).digest("hex");
}

function orderedRowsFingerprint(rows, config) {
  const indexes = rows.map((_, index) => index);
  const signatures = indexes.map((index) => {
    const row = rows[index] || [];
    return config.keyColumnIndexes.map((columnIndex) => {
      if (columnIndex === config.dateColumnIndex)
        return parseDateValue(row[columnIndex]) || "";
      if (columnIndex === config.amountColumnIndex)
        return Math.round(toNumber(row[columnIndex]) * 100);
      return normalizeText(row[columnIndex]);
    });
  });
  return crypto
...
function indexesOutside(rowCount, excludedIndexes) {
...
function appendedRange(sheetName, previousRowCount, deleteCount, appendCount, lastColumn) {
  if (!appendCount) return null;
  const startRow = previousRowCount - deleteCount + 1;
  const endRow = startRow + appendCount - 1;
  return `${sheetName}!A${startRow}:${lastColumn}${endRow}`;
}
...
async function run() {
  ...
  const productSelectorFingerprintConfig = {
    dateColumnIndex: 0,
    amountColumnIndex: 5,
    keyColumnIndexes: [0, 1, 2, 3, 4, 5],
  };
  const vendorSelectorFingerprintConfig = {
    dateColumnIndex: 0,
    amountColumnIndex: 4,
    keyColumnIndexes: [0, 1, 2, 3, 4, 5, 6],
  };
  const productFullFingerprintConfig = {
    dateColumnIndex: 0,
    amountColumnIndex: 5,
    keyColumnIndexes: Array.from({ length: 11 }, (_, index) => index),
  };
  const vendorFullFingerprintConfig = {
    dateColumnIndex: 0,
    amountColumnIndex: 4,
    keyColumnIndexes: Array.from({ length: 12 }, (_, index) => index),
  };

  secureLog(
    `Linhas a remover: ${PRODUCT_SHEET}=${productDeleteIndexes.length}; ${VENDOR_SHEET}=${vendorDeleteIndexes.length}`,
  );
  const beforeProduct = windowMetrics(currentProductRows, productDeleteIndexes, 5);
  const beforeVendor = windowMetrics(currentVendorRows, vendorDeleteIndexes, 4);
  const beforeProductOutsideFingerprint = rowsFingerprint(
    currentProductRows,
    indexesOutside(currentProductRows.length, productDeleteIndexes),
    productSelectorFingerprintConfig,
  );
  const beforeVendorOutsideFingerprint = rowsFingerprint(
    currentVendorRows,
    indexesOutside(currentVendorRows.length, vendorDeleteIndexes),
    vendorSelectorFingerprintConfig,
  );
  secureLog(`Validacao anterior: produtos=${beforeProduct.count}; vendedores=${beforeVendor.count}`);

  // The physical row indexes are part of the delete request. Re-read immediately
  // before writing and abort if another writer changed/reordered the sheet.
  const preflightRows = await readSelectorRows(sheets);
  if (
    orderedRowsFingerprint(preflightRows.productRows, productSelectorFingerprintConfig) !==
      orderedRowsFingerprint(currentProductRows, productSelectorFingerprintConfig) ||
    orderedRowsFingerprint(preflightRows.vendorRows, vendorSelectorFingerprintConfig) !==
      orderedRowsFingerprint(currentVendorRows, vendorSelectorFingerprintConfig)
  ) {
    throw new Error(
      "Planilha alterada por outro processo durante a sincronizacao; escrita cancelada com seguranca",
    );
  }
  ...
  try {
    ...
    writeError = error;
    secureLog("Resposta da escrita foi ambigua; validando o estado final antes de decidir");
  }

  const {
    productRows: afterProductRows,
    vendorRows: afterVendorRows,
  } = await readSelectorRows(sheets);
  const afterProductIndexes = rowIndexesForWindow(afterProductRows, { startKey, endKey, companyColumnIndex: 1, companies });
  const afterVendorIndexes = rowIndexesForWindow(afterVendorRows, { startKey, endKey, companyColumnIndex: 6, companies });
  const afterProduct = windowMetrics(afterProductRows, afterProductIndexes, 5);
  const afterVendor = windowMetrics(afterVendorRows, afterVendorIndexes, 4);
  const expectedProductAmount = Math.round(productValues.reduce((sum, row) => sum + toNumber(row[5]), 0) * 100) / 100;
  const expectedVendorAmount = Math.round(vendorValues.reduce((sum, row) => sum + toNumber(row[4]), 0) * 100) / 100;

  const productTailRange = appendedRange(
    PRODUCT_SHEET,
    currentProductRows.length,
    productDeleteIndexes.length,
    productValues.length,
    PRODUCT_LAST_COLUMN,
  );
  const vendorTailRange = appendedRange(
    VENDOR_SHEET,
    currentVendorRows.length,
    vendorDeleteIndexes.length,
    vendorValues.length,
    VENDOR_LAST_COLUMN,
  );
  const tailRanges = [];
  const productTailIndex = productTailRange ? tailRanges.push(productTailRange) - 1 : -1;
  const vendorTailIndex = vendorTailRange ? tailRanges.push(vendorTailRange) - 1 : -1;
  const tailValues = tailRanges.length
    ? await sheets.getValuesBatch(tailRanges, {
        valueRenderOption: "UNFORMATTED_VALUE",
        dateTimeRenderOption: "SERIAL_NUMBER",
      })
    : [];
  const actualProductFullRows = productTailIndex >= 0 ? tailValues[productTailIndex] : [];
  const actualVendorFullRows = vendorTailIndex >= 0 ? tailValues[vendorTailIndex] : [];

  const actualProductFingerprint = rowsFingerprint(
    actualProductFullRows,
    actualProductFullRows.map((_, index) => index),
    productFullFingerprintConfig,
  );
  const actualVendorFingerprint = rowsFingerprint(
    actualVendorFullRows,
    actualVendorFullRows.map((_, index) => index),
    vendorFullFingerprintConfig,
  );
  const expectedProductFingerprint = rowsFingerprint(
    productValues,
    productValues.map((_, index) => index),
    productFullFingerprintConfig,
  );
  const expectedVendorFingerprint = rowsFingerprint(
    vendorValues,
    vendorValues.map((_, index) => index),
    vendorFullFingerprintConfig,
  );
  const afterProductOutsideFingerprint = rowsFingerprint(
    afterProductRows,
    indexesOutside(afterProductRows.length, afterProductIndexes),
    productSelectorFingerprintConfig,
  );
  const afterVendorOutsideFingerprint = rowsFingerprint(
    afterVendorRows,
    indexesOutside(afterVendorRows.length, afterVendorIndexes),
    vendorSelectorFingerprintConfig,
  );
  if (
    afterProduct.count !== productValues.length ||
    afterVendor.count !== vendorValues.length ||
    afterProduct.amount !== expectedProductAmount ||
    afterVendor.amount !== expectedVendorAmount ||
    actualProductFullRows.length !== productValues.length ||
    actualVendorFullRows.length !== vendorValues.length ||
    actualProductFingerprint !== expectedProductFingerprint ||
    actualVendorFingerprint !== expectedVendorFingerprint ||
    afterProductOutsideFingerprint !== beforeProductOutsideFingerprint ||
    afterVendorOutsideFingerprint !== beforeVendorOutsideFingerprint
  ) {
    if (writeError) throw writeError;
    throw new Error("Validacao apos escrita falhou; a proxima execucao idempotente deve reparar a janela");
  }
  if (writeError) secureLog("Escrita confirmada pela validacao apos resposta ambigua");
...
if (require.main === module) {
  run().catch((err) => {
    secureLog(formatPublicError(err), true);
    process.exit(1);
  });
}
